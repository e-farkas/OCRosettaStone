...
def main(img_name):
    ...
    #Note: could combine video capture code here
    ...
    for i in indices:
        ...
        for j in range(4):
            ...
            x_points.append(vertices[j][0])
            y_points.append(vertices[j][1])
            #x_points.append(vertices[(j + 1) % 4][0])
            #y_points.append(vertices[(j + 1) % 4][1])
    xmin = min(x_points)
    xmax = max(x_points)
    ymin = min(y_points)
    ymax = max(y_points)
    ...
    ymin = int(ymin - y_padding)
    ymax = int(ymax + y_padding)
    ...
    cv.imwrite("cropped_py.jpg", cropped)
        #exit()
